...
const TablaPaciente = () => {
  ...
  const { obtenerPaciente, startCrearPaciente, startEditarPaciente } = usePacientesStore();
  ...
  useEffect(() => {
    const fetchData = async () => {
      let response = await obtenerPaciente();
  ...
  const btnGuardarPaciente = async() => {
    ...
    console.log('Ficha Medica:', fichaMedica);
    if(rutValido){
      const guardar = await startCrearPaciente({   
  ...
  const editarPaciente  = async() => {
    if(rutValido){
      const guardar = await startEditarPaciente({   
        fichaMedica:fichaMedica,
        rut:rutInput,
        nombre:nombre,
        edad:edad,
  ...
  const editarSeleccion = async() => {
    if (selectedRow?.nombre !== undefined) {
      ...
      setRutInput(selectedRow?.rut);
      setEdad(selectedRow?.edad);
      setFichaMedica(selectedRow?.fichaMedica);
      setOpen2(true);
  ...
  const nuevoProcedimiento = () => {
    if (selectedRow?.nombre !== undefined) {
      ...
      const dataPersona = {
        ...
        rut: selectedRow?.rut,
        fichaMedica: selectedRow?.fichaMedica,
